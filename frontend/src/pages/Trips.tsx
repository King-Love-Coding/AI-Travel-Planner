...
const TripDetails: React.FC = () => {
  ...
  // Add getTripImage function (same as Trips page)
  const getTripImage = (destination: string): string => {
  ...
  const fetchTripDetails = async () => {
    ...
    try {
      ...
      // Initialize with basic trip data - ADD IMAGE HANDLING HERE
      const tripDetails: Trip = {
        ...tripData,
        // Add status and image handling like in Trips page
      ...
      try {
        const members = await apiService.getTripMembers(tripId);
        tripDetails.members = members || [];
      } catch (memberError) {
        console.warn('Could not fetch members:', memberError);
        tripDetails.members = [];
      }

      try {
        const activities = await apiService.getTripActivities(tripId);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-4">
          <div className="bg-white rounded-2xl shadow-lg p-8 border border-red-200">
            ...
            <p className="text-red-600 mb-6">{error || 'The trip you are looking for does not exist.'}</p>
